...
X1 S DIC("S")="I $$CFLTF^DGREGDD(Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".3;1",DV="SX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C2^DVBAXA5"
 S DU="Y:YES;N:NO;"
 G RE
...
X2 S DFN=DA D EV^DGLOCK I $D(X),X="Y" D VET^DGLOCK
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".3;2",DV="NJ3,0X",DU="",DLB="SERVICE CONNECTED PERCENTAGE",DIFLD=.302
 S DE(DW)="C3^DVBAXA5"
 G RE
...
X3 S DFN=DA D EV^DGLOCK Q:'$D(X)  K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X),$D(^DPT(DA,.3)),$P(^(.3),U,1)'="Y" W !?4,*7,"Only applies to service-connected applicants." K X
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="TYPE;1",DV="P391'",DU="",DLB="TYPE",DIFLD=391
...
C4F2 Q
X4 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="VET;1",DV="SX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C5^DVBAXA5"
 S DU="Y:YES;N:NO;"
 G RE
...
C6S S X="" G:DG(DQ)=X C6F1 K DB
 D ^DVBAXA6
C6F1 N X,X1,X2 S DIXR=840 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
X6 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".52;5",DV="SXR",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525

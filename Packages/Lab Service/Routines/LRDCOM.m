LRDCOM ;SLC/BA - REPORT OF DELETED OR EDITED COMMENTS ;2/19/91  10:32 ;
 ;;5.2;LAB SERVICE;;Sep 27, 1994;Build 25
...
SETUP ;S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3),LRIDT0=^LR(LRDFN,"CH",LRIDT,0),LRACC=$P(LRIDT0,U,6),Y=$P(LRIDT0,U) D DD^LRX S LRTIME=Y D PT^LRX
 ...
 . S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3),LRIDT0=^LR(LRDFN,"CH",LRIDT,0),LRACC=$P(LRIDT0,U,6),Y=$P(LRIDT0,U) D DD^LRX S LRTIME=Y D PT^LRX
 . I $Y>(IOSL-7) D:$E(IOST,1,2)="C-" WAIT Q:LREND  D HDR
 . W !,LRACC,?15,PNM,?35,"DOB: ",$P($G(VADM(3)),U,2)
 . W !,?15,$G(VA("MRN",0))_" :",SSN
 . W !,?15,"Collected: ",LRTIME,?35
 E  D
 .S LRDPF=$P(^LR(LRDFN,0),U,2),DFN=$P(^(0),U,3),LRIDT0=^LR(LRDFN,"CH",LRIDT,0),LRACC=$P(LRIDT0,U,6),Y=$P(LRIDT0,U) D DD^LRX S LRTIME=Y D PT^LRX
 .I $Y>(IOSL-7) D:$E(IOST,1,2)="C-" WAIT Q:LREND  D HDR

VFDXTPT ;DSS/LM/RAC - Pointer Tool ;04/26/2013@1230
 ;;2011.1.2;DSS,INC VXVISTA OPEN SOURCE;;01 Dec 2009;Build 2
...
LMA ;[Public]
 ...
 S VFDFILE=+Y
 S VFDXFILE=$P(Y,U,2),VFDFGL=$$ROOT^DILFD(VFDFILE,,1)
...
A1P ;[Private] Called by VFDA1 - Process one pointer
 ...
 N VFDROOT S VFDROOT=$P($$ROOT^DILFD(VFDFILE),U,2)
 N VFDCMD,VFD S VFDCMD=$$VFDCMD
...
ADSP(VFDRSLTS) ;[Private] Display results - Called by PTR
 ...
 ....I @VFDRSLTS@(VFDFILE,VFDA,VFDPFIL,VFDPFLD) W "  [V]"
 ....S (CNT,VFDIENS)=0
...
C1P ;[Private] Called by LMC1 - Process one pointer
 ...
 ;
VFDCMD() ;
 Q "I $S($G(VFDVAL):VFDVAL=VFDA,1:1) S VFD=$P($G(^(SIS(SISN),VFDS)),U,VFDP) S:VFD=VFDA!(VFD=(VFDA_"";""_VFDROOT)) @VFDRSLTS@(VFDFILE,VFDA,VFDPFIL,VFDPFLD)=VFD["";"",^(VFDPFLD,$$CIENS^SISFIND(SISIENS))="""""

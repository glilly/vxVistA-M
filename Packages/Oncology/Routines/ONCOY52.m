ONCOY52 ; GENERATED FROM 'ONCOY52' PRINT TEMPLATE (#862) ; 04/10/06 ; (FILE 165.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^ONCO(165.5,D0,0)) D N:$X>34 Q:'DN  W ?34 S Y=$P(X,U,4) W:Y]"" $S($D(DXS(6,Y)):DXS(6,Y),1:Y)
 ...
 S X=$G(^ONCO(165.5,D0,0)) D N:$X>34 Q:'DN  W ?34 S Y=$P(X,U,21) W:Y]"" $S($D(DXS(7,Y)):DXS(7,Y),1:Y)
 ...
 S X=$G(^ONCO(165.5,D0,0)) D N:$X>34 Q:'DN  W ?34 S Y=$P(X,U,26) W:Y]"" $S($D(DXS(8,Y)):DXS(8,Y),1:Y)

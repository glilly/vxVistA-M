APCDDVC ; IHS/CMI/TUCSON - VISIT REVIEW REPORT ; [ 01/20/04  7:39 AM ]
 ;;2.0;IHS RPMS/PCC Data Entry;**2,4,7**;MAR 09, 1999
 ;
 ;
GETCHART ;get chart number
 ...
 I APCDCLOC="" S APCDHRN="NONE",APCDCLOC=DUZ(2) Q
C2  ;
 Q:APCDCSRT'="T"
...
ZERO ; If no dependent entries, save information.
 K APCDERR
 S APCDEC=1
 D ZERO^APCDRV I $D(APCDERR) D GETCHART S ^XTMP("APCDDV",APCDJOB,APCDBT,"ZERO",APCDCLOC,APCDCLIN,APCDHRN,APCDVSIT)=""
 D XIT
 Q
PPPV ; See if Purpose of Visit and Providers entered correctly
 ...
 I $D(APCDERR) D GETCHART S APCDX=0 F  S APCDX=$O(APCDERR(APCDX)) Q:APCDX'=+APCDX  D
...
CHKCLN ;
 ...
 I APCDF=1 F APCDII=1:1:APCDI S ^XTMP("APCDDV",APCDJOB,APCDBT,"MRG",APCDCLOC,APCDCLIN,APCDHRN,APCDVSIT,+APCDKT(APCDII))="",^XTMP("APCDDV",APCDJOB,APCDBT,"AM",APCDCLOC,APCDCLIN,APCDHRN,+APCDKT(APCDII))=""
 Q
TXER ; Create transaction error report
 ...
 I $D(APCDERR) D GETCHART S APCDX=0 F  S APCDX=$O(APCDERR(APCDX)) Q:APCDX'=+APCDX  D SETTXER
 D XIT
 Q
SETTXER ;
...
INPT ;
 ...
 S APCDEC=1
 S APCDVREC=^AUPNVSIT(APCDVSIT,0) D ^APCDRV K APCDVREC
 I $D(APCDERR) D GETCHART S APCDX=0 F  S APCDX=$O(APCDERR(APCDX)) Q:APCDX'=+APCDX  S ^XTMP("APCDDV",APCDJOB,APCDBT,"INPT",APCDCLOC,APCDCLIN,APCDHRN,APCDVSIT,APCDX)=$P(^APCDERR(APCDERR(APCDX),0),U)_"-"_$P(^(0),U,2)
 D XIT
 Q

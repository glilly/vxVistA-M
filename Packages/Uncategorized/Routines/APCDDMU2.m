APCDDMU2 ; IHS/CMI/LAB - EDITS FOR AUPNVSIT (VISIT:9000010) 24-MAY-1993 ; [ 01/08/04  10:43 AM ]
...
PPD ;EP
 ...
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update PPD.",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVSK("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVSK(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a PPD  on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Skin Test Entry for PPD.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
EKG ;EP
 K APCDVSIT
 ...
 D EVSIT ;get event visit
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update EKG.",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVDXP("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVDXP(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a EKG on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Diagnostic Procedure Entry for EKG.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
LAB ;EP
...
LAB1 ;
 ...
 D EVSIT ;get event visit
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update lab .",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVLAB("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVLAB(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a  ",$P(^LAB(60,APCDMTYP,0),U),"  on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Lab Entry for lab test.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
...
EDUC1 ;
 ...
 D EVSIT ;get event visit
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update pt education topic.",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVPED("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVPED(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a  ",$P(^AUTTEDT(APCDMTYP,0),U),"  on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Patient Ed Entry for Education topic.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
MED ;EP
...
MED1 ;
 ...
 I G W !!,"Already have a  ",$P(^PSDRUG(APCDMTYP,0),U),"  on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Medication Entry for lab test.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
BP ;EP
 K APCDVSIT
 ...
 I G W !!,"Already have a height of ",$P(APCDREC,U,20)," on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Measurement Entry for BP.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 Q
...
SGHF ;EP
 ...
 D EVSIT ;get event visit
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update self monitoring health factor.",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVHF("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVHF(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a health factor of ",$P(^AUTTHF($P(^APCDDMUP(APCDDA,11),U,7),0),U)," on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 ...
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Health Factor Entry for Self Monitoring of Blood Glucose.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
...
TBHF ;EP
 ...
 S APCDMCAT=$P(^AUTTHF(APCDMTYP,0),U,3)
 D EVSIT ;get event visit
 I '$G(APCDVSIT) W !!,"Could not Create PCC Visit when attempting to update tb health factor.",! S APCDDMER=1 Q
 S (X,G)=0 F  S X=$O(^AUPNVHF("AD",APCDVSIT,X)) Q:X'=+X!(G)  I $P(^AUPNVHF(X,0),U)=APCDMTYP S G=1
 I G W !!,"Already have a health factor of ",$P(^AUTTHF($P(APCDREC,U,10),0),U)," on Visit Date ",$$FMTE^XLFDT($P(^AUPNVSIT(APCDVSIT,0),U)) S APCDDMER=1 Q
 K APCDALVR
 S APCDALVR("APCDPAT")=APCDDMPT
 S APCDALVR("APCDVSIT")=APCDVSIT
 S APCDALVR("APCDATMP")="[APCDALVR 9000010.23 (ADD)]"
 S APCDALVR("APCDTHF")="`"_APCDMTYP
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) W !!,"Error creating V Health Factor Entry for TB.  PCC not updated.",! S APCDDMER=1
 K APCDALVR
 S APCDHSE="",X=0 F  S X=$O(^AUPNHF("AC",APCDDMPT,X)) Q:X'=+X!(APCDHSE)  I $P(^AUTTHF($P(^AUPNHF(X,0),U),0),U,3)=APCDMCAT S APCDHSE=X
 I APCDHSE D  Q
 .D ^XBFMK K DIADD
 .S DA=APCDHSE,DIE="^AUPNHF(",DR=".01///`"_APCDMTYP_";.03////"_DT D ^DIE
 .I $D(Y) W !!,"Error updating Health Status entry for TB." S APCDDMER=1
 .D ^XBFMK
 D ^XBFMK
 S X=APCDMTYP,DIC("DR")=".02////"_APCDDMPT_";.03////"_DT,DIC(0)="L",DIADD=1,DLAYGO=9000019,DIC="^AUPNHF(" D FILE^DICN
 I Y=-1 W !!,"Error adding health status entry for TB." S APCDDMER=1
 D ^XBFMK K DIADD
 Q

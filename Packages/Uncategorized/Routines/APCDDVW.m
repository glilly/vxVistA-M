APCDDVW ; IHS/CMI/TUCSON - NO DESCRIPTION PROVIDED ; [ 11/24/03  2:31 PM ]
 ;;2.0;IHS RPMS/PCC Data Entry;**4,7**;MAR 09, 1999
 ;
START ;
 S %DT="",X="T" D ^%DT S DT=Y X ^DD("DD") S APCDDT=Y
 S APCD80S="-------------------------------------------------------------------------------"
...
DE2 ;
 ...
 .S APCDK12N=APCDVDG_APCDVDFN_",12)"
 .I $D(@(APCDK12N)) S APCDK12N=@(APCDK12N)
 .S APCDK16N="" I APCDVFLE=9000010.09 S APCDK16N=APCDVDG_APCDVDFN_",16)" I $D(@(APCDK16N)) S APCDK16N=@(APCDK16N)
 .I $P(APCDK16N,U)]"" W ?5,$$VAL^XBDIQ1(APCDVFLE,APCDVDFN,.01),?40,$P(APCDK16N,U),!
 .I $P(APCDK12N,U,13)]"" W ?5,$$VAL^XBDIQ1(APCDVFLE,APCDVDFN,.01),?40,$$VAL^XBDIQ1(APCDVFLE,APCDVDFN,1213),!
 Q
 ;
MRG ;
 D MRG^APCDDVW1

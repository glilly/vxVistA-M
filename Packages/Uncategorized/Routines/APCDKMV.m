APCDKMV ; IHS/CMI/TUCSON - LINK DIF DAY MICROBIOLOGY VISITS ; [ 08/26/03  1:58 PM ]
 ;;2.0;IHS RPMS/PCC Data Entry;**7**;MAR 09, 1999
 ;
 ; -- ** THANKS TO LINDA FELS
 ;    ** FOR THE ORIGINAL ROUTINE.
 ; -- This routine takes visits with only v microbiology entries and completes
...
QUEUE ;EP; entry point to run linker in background
 I '$D(ZTQUEUED) W !!,"Orphaned V Microbiology Linker is being queued to run in the background!",!,"Dates of the run will be automatically calculated based on the PCC delay",!,"value.",!
 NEW DELAY,X1,X2,X
 Q:'$$LABTECH  Q:'$$LABCLN
...
MANUAL ;EP; entry to run linker manually
 ...
 D ^XBCLS W !!?20,"FIX UNLINKED MICROBIOLOGY VISITS",!!
 ;
 I '$$LABTECH D  Q
 . W !!,$C(7),"You do not have a generic Lab Technician provider entry"
 . W !,"in your database.  Cannot run fix for unlinked V MICROBIOLOGY visits."
...
START(APCDZMOD) ; begin of linker logic
 ...
 .. Q:'$D(^AUPNVMIC("AD",APCDV))  ;not a lab visit
 .. S X=$$VALI^XBDIQ1(9000010,APCDV,.07) I (X'="A"),(X'="S") Q
 .. ;S X=$O(^AUPNVMIC("AD",APCDV,0)) I X,$$GET1^DIQ(9000010.25,X,1202)\1=APCDT\1 Q  ;if ordered on same date, quit
 .. S X=$O(^AUPNVMIC("AD",APCDV,0)) I X,($P($P($G(^AUPNVMIC(X,12)),U,11),".")=$P(APCDT,".")) Q
 .. ;
 ...
 ... K DD,D0,DO,DINUM,DIC,DA,DR S DIC(0)="EL",DIC="^APCDLLOG(",DLAYGO=9001001.7,DIADD=1,X=APCDV,DIC("DR")=".02////"_DT_";.03///M" D FILE^DICN K DIC,DR,DIE,DIADD,DLAYGO,X,D0
...
LINK(APCDVST) ; -- find orig visit and set link
 ...
 S APCDLAB=$O(^AUPNVMIC("AD",APCDVST,0)) Q:'APCDLAB
 K APCDX D ENP^XBDIQ1(9000010.25,APCDLAB,".02;1202;1211","APCDX(","I")

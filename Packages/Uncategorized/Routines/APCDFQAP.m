APCDFQAP ; IHS/CMI/TUCSON - PRINT DE QA ; [ 06/17/02  10:18 AM ]
...
START ;
 ...
 S APCDPG=0 D HEAD I '$D(^XTMP("APCDFQA",APCDJOB,APCDBT)) W !,"No visits to report",! G DONE
 K APCDQUIT
...
WRT ;
 ...
 W !!,APCDHRN,?12,APCDDATE S APCDPOVD=APCDPOVA(1) W ?33,$P(^ICD9(+^AUPNVPOV(APCDPOVD,0),0),U)
 S APCDNQ=$P(^AUPNVPOV(APCDPOVD,0),U,4) W:APCDNQ]"" ?40,$E($P(^AUTNPOV(APCDNQ,0),U),1,40),!?40,"[",$E($P(^ICD9(+^AUPNVPOV(APCDPOVD,0),0),U,3),1,42),"]"
 D POVW
 Q
POVW ;
 S APCDPOVN=1 F  S APCDPOVN=$O(APCDPOVA(APCDPOVN)) Q:APCDPOVN=""  D
 .S APCDPOVD=APCDPOVA(APCDPOVN) W !?33,$P(^ICD9(+^AUPNVPOV(APCDPOVD,0),0),U) S APCDNQ=$P(^AUPNVPOV(APCDPOVD,0),U,4) W:APCDNQ]"" ?40,$E($P(^AUTNPOV(APCDNQ,0),U),1,40)
 .W !?40,"[",$E($P(^ICD9(+^AUPNVPOV(APCDPOVD,0),0),U,3),1,42),"]"
 Q

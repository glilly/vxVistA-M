SISP6CL ;SIS/LM - Clone and optionally modify routine DGRP6CL
...
CLLST(DFN,DGCONF,DGPOSS,DGMSE) ;
 ...
 . F FRTO=1,0 S $P(DGCONF(DGCONFX),U,$S(FRTO:1,1:2))=$$GETDT^DGRPMS(DFN,DGCONFX,FRTO) I FRTO=0 D CKDT^SISP6CL1(.DGCONF,.DGMSE,.DGPOSS)
 . S $P(DGCONF(DGCONFX),U,3)=$G(DGZ("SITE",DGZ0))
 . S $P(DGCONF(DGCONFX),U,4)=$G(DGZ("LOCK",DGZ0))
 F DGCONF="OEF","OIF","UNK" D CKDT^SISP6CL1(.DGCONF,.DGMSE,.DGPOSS)
 F DGCONF="VIET","LEB","GREN","PAN","GULF","SOM","YUG" F FRTO=1,0 S $P(DGCONF(DGCONF),U,$S(FRTO:1,1:2))=$$GETDT^DGRPMS(DFN,DGCONF,FRTO) I FRTO=0 D CKDT^SISP6CL1(.DGCONF,.DGMSE,.DGPOSS)
 Q
 ;
...
EN1 ; Entry from conf subscreen off reg screen 6
 ...
 . S DG=$$ISVALID^SISP6CL2(.DGCONF,.DGPOSS)
 . S DGEG=DGEG+1
 ...
 D LOOPCNF^SISP6CL1(.DGCONF,.DGPOSS,.DIR)
 S DGCT=DGCT+1,DIR("A",DGCT)=" "
 ...
 S DIR("?")="^D HELP^SISP6CL1($P(DIR(0),U,2))"
 S DIR("B")="QUIT"
 D ^DIR K DIR
 ...
 . I '$G(DGCONF(DGCONF,"OK")),$G(DGCONF(DGCONF,"VEDIT"))'=2 D ADDCFL^SISP6CL1(DFN,DGY1,DGCONF) Q  ; Add new only valid action
 ...
 . Q:$D(DTOUT)!$D(DUOUT)
 . I Y="A" D ADDCFL^SISP6CL1(DFN,DGY1,DGCONF) Q
 . I Y="E" D
 .. N DGXREF,IEN,DIR,X,Y
 .. I DGCONF(DGCONF,"OK")=1 S IEN=+$O(DGCONF(DGCONF,"OK",0)) I IEN D EDCFL^SISP6CL1(DFN,IEN,$G(DGCONF(DGCONF,"VEDIT"))) Q
 ...
 .. S DIR(0)=DIR(0)_"Q:QUIT"
 .. D ^DIR K DIR
 .. I Y D EDCFL^SISP6CL1(DFN,+$G(DGXREF(+Y)),$G(DGCONF(DGCONF,"VEDIT")))
 G EN1
 ;
QUIT Q
 ;

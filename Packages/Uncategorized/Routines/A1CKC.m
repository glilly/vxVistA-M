A1CKC ; GENERATED FROM 'A1CK VARO/DHCP' INPUT TEMPLATE(#1505), FILE 2;05/20/09
 D DE G BEGIN
...
X1 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
2 S DQ=3 ;@10
3 S DW="VET;1",DV="SX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C3^A1CKC"
 S DU="Y:YES;N:NO;"
 S Y="Y"
...
X3 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".3;1",DV="SX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C4^A1CKC"
 S DU="Y:YES;N:NO;"
 S Y="Y"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X4 S DFN=DA D EV^DGLOCK I $D(X),X="Y" D VET^DGLOCK
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".3;2",DV="NJ3,0X",DU="",DLB="SERVICE CONNECTED PERCENTAGE",DIFLD=.302
 S DE(DW)="C5^A1CKC"
 S X=PER
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X6 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="TYPE;1",DV="P391'",DU="",DLB="TYPE",DIFLD=391
...
X8 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
9 S DQ=10 ;@20
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW="VET;1",DV="SX",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C10^A1CKC"
 S DU="Y:YES;N:NO;"
 S Y="Y"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)

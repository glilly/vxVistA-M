APCDKPV ; IHS/CMI/TUCSON - LINK DIF DAY RX VISITS ; [ 07/10/02  5:45 AM ]
 ;;2.0;IHS RPMS/PCC Data Entry;**5**;MAR 09, 1999
 ;
 ; -- ** THANKS TO LINDA FELS, ANMC COMPUTER DEPARTMENT
 ;    ** FOR THIS ROUTINE.
 ; -- This routine takes visits with only v med entries and completes
...
QUEUE ;EP; entry point to run linker in background
 I '$D(ZTQUEUED) W !!,"Orphaned Pharmacy Linker is being queued to run in the background!",!,"Dates of the run will be automatically calculated based on the PCC delay",!,"value.",!
 NEW DELAY,X1,X2,X
 Q:'$$MEDTECH  Q:'$$MEDCLN
...
MANUAL ;EP; entry to run linker manually
 ...
 D ^XBCLS W !!?20,"FIX UNLINKED PHARMACY VISITS",!!
 ;
 I '$$MEDTECH D  Q
 . W !!,$C(7),"You do not have a generic Pharmacist provider entry"
 . W !,"in your database.  Cannot run fix for unlinked pharmacy visits."
 . S DIR(0)="E",DIR("A")="Press ENTER" D ^DIR
 ;
 I '$$MEDCLN D  Q
 . W !!,$C(7),"You do not have PHARMACY as a clinic stop.  Cannot run"
 . W !,"fix for unlinked pharmacy visits."
 . S DIR(0)="E",DIR("A")="Press ENTER" D ^DIR
 ;
...
START(APCDZMOD) ; begin of linker logic
 ...
 .. Q:'$D(^AUPNVMED("AD",APCDV))  ;not a med visit
 .. S X=$$VALI^XBDIQ1(9000010,APCDV,.07) I (X'="A"),(X'="S") Q
 .. ;S X=$O(^AUPNVMED("AD",APCDV,0)) I X,$$GET1^DIQ(9000010.09,X,1202)\1=APCDT\1 Q  ;if ordered on same date, quit
 .. S X=$O(^AUPNVMED("AD",APCDV,0)) I X,($P($P($G(^AUPNVMED(X,12)),U,11),".")=$P(APCDT,".")) Q
 .. ;
 ...
 ... K DD,D0,DO,DINUM,DIC,DA,DR S DIC(0)="EL",DIC="^APCDLLOG(",DLAYGO=9001001.7,DIADD=1,X=APCDV,DIC("DR")=".02////"_DT_";.03///P" D FILE^DICN K DIC,DR,DIE,DIADD,DLAYGO,X,D0
...
LINK(APCDVST) ; -- find orig visit and set link
 NEW APCDX,APCDMED,ORDT,ORDPRV,DFN,DATE,PRV,ORDV,LINK
 ;
 ; -- get first rx entry for visit
 S APCDMED=$O(^AUPNVMED("AD",APCDVST,0)) Q:'APCDMED
 K APCDX D ENP^XBDIQ1(9000010.14,APCDMED,".02;1202;1211","APCDX(","I")
...
STUFF(AUPNVSIT,APCDZMOD) ; -- stuff pov and provider
 ...
 . S DIE="^AUPNVSIT(",DA=AUPNVSIT,DR=".08////"_$$MEDCLN D ^DIE
 ;
 ; -- create purpose of visit entry
 ; -- uses rad draw (ICD code V68.1)
 K APCDALVR
 S APCDALVR("APCDPAT")=DFN,APCDALVR("APCDVSIT")=AUPNVSIT
 S APCDALVR("APCDATMP")="[APCDALVR 9000010.07 (ADD)]"
 S APCDALVR("APCDTPOV")="V68.1"
 S APCDALVR("APCDTNQ")="PHARMACY REFILL"
 D ^APCDALVR
 I $D(APCDALVR("APCDAFLG")) Q
 ;
 ; -- create v provider entry
 ; -- uses rad tech (09) plus affiliation based on PCC site type
 K APCDALVR
 S APCDALVR("APCDPAT")=DFN,APCDALVR("APCDVSIT")=AUPNVSIT
 S APCDALVR("APCDATMP")="[APCDALVR 9000010.06 (ADD)]"
 I $$PROVP=6 S X=$O(^DIC(6,"GIHS",$$MEDTECH,0)) Q:'X
 I $$PROVP=200 S X=$O(^VA(200,"GIHS",$$MEDTECH,0)) Q:'X
...
RVDT(X) ; -- returns reverse date 
 Q 9999999-X
 ;
MEDTECH() ; -- returns code if rad tech entry exists in file 200
 NEW SITE,CODE
 S SITE=$$VALI^XBDIQ1(9001000,DUZ(2),.04) I SITE="" Q 0
 S CODE=$P($T(@SITE),";;",2)_"09999"
...
PROVP() ; -- returns pointer file # for providers
 ...
 Q $S(X["200":200,1:6)
 ;
MEDCLN() ; -- returns ien for rx clinic code
 Q +$O(^DIC(40.7,"C","39",0))
 ;

APCDRVH1 ; IHS/CMI/TUCSON - REVIEW HOSPITALIZATIONS CONT. ; [ 01/04/00  11:05 AM ]
...
CHKDXOP ; CHECK DX VS OPERATIONS AND CHECK FOR DUPLICATE POVS
 ...
 K APCDDUPO,APCDVPRC,APCDOPP S APCDPX=0 F  S APCDPX=$O(^AUPNVPRC("AD",APCDVSIT,APCDPX)) Q:APCDPX=""  S APCDOPDX=$P(^AUPNVPRC(APCDPX,0),U,5) S:APCDOPDX]"" APCDVPRC(APCDPX)=APCDOPDX_"^"_$P(^ICD9(APCDOPDX,0),U) D CHKDXOP2
 ...
 S O=0 F  S O=$O(^AUPNVPOV("AD",APCDVSIT,O)) Q:O'=+O!(C>8)  I $P(^AUPNVPOV(O,0),U,12)'="P" S C=C+1 D GETPOV ;IHS/CMI/LAB - FIX # of diagnoses per Cheryl Chase
...
XIT ;
 ...
 Q
GETPOV ;
 S APCDRVH1("ICD PTR")=$P(^AUPNVPOV(O,0),U),APCDRVH1("ICD")=$P(^ICD9(APCDRVH1("ICD PTR"),0),U)
 S APCDRVH1("POV",C)=APCDRVH1("ICD")
 Q
GETPRC ;
 S APCDRVH1("O PTR")=$P(^AUPNVPRC(O,0),U)
 S (APCDRVH1("OH"),N)="",APCDRVH1("OH")=$P(^ICD0(APCDRVH1("O PTR"),0),U)
 S APCDRVH1("ICD")="",APCDRVH1("ICD PTR")=$P(^AUPNVPRC(O,0),U,5) I APCDRVH1("ICD PTR")="" S APCDE="E336" D ERR Q
 S APCDRVH1("ICD")=$P(^ICD9(APCDRVH1("ICD PTR"),0),U)
...
DUPEOP ;WARNING RE:  DUPLICATE OPERATIONS USED;ACCEPT REQUIRED
 ...
 ;
C567 ;CHK DX VS OPS
 S APCD1=0 F  S APCD1=$O(APCDVPRC(APCD1)) Q:APCD1=""  S APCD2=$P(APCDVPRC(APCD1),U,2),APCDOPC=$P(^ICD0($P(^AUPNVPRC(APCD1,0),U),0),U) D
 ...
 S APCD1=0 F  S APCD1=$O(APCDPOVS(APCD1)) Q:APCD1=""  S APCDDX=$P(^AUPNVPOV(APCD1,0),U) I $D(^APCDINPT(6,11,"AC",$P(^ICD9(APCDDX,0),U))) D C6ERR
 Q
C6ERR ;
 I '$D(APCDVPRC) S APCDE="E333",APCDE("FILE")=9000010,APCDE("ENTRY")=APCDVSIT D ERR Q
 K APCDFOUN S APCD2=0 F  S APCD2=$O(APCDVPRC(APCD2)) Q:APCD2=""  S APCDOP=$P(^ICD0($P(^AUPNVPRC(APCD2,0),U),0),U) I $D(^APCDINPT(6,12,"AC",APCDOP)),$P(APCDVPRC(APCD2),U,1)=APCDDX S APCDFOUN=1

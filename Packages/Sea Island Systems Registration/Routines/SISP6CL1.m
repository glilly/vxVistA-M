SISP6CL1 ;SIS/LM - Clone and optionally modify routine DGRP6CL1
...
EDCFL(DFN,IEN,VEDIT) ; Edit OEF/OIF conflict from/to dates only or delete entry
 ...
 S DA(1)=DFN,DA=IEN,DIE="^DPT("_DA(1)_",.3215,",DR=".05///NOW;@10;.01;I X'=DIPA(.01) S Y=""@50"";.02R;.03R;S Y=""@99"";@50;D NOCHG^SISP6CL1;.01////"_$G(DIPA(.01))_";S Y=""@10"";@99"
 D ^DIE
 Q
 ;
...
LOOPCNF(DGCONF,DGPOSS,DIR) ; Loop thru non-OEF/OIF conflicts
 ...
 . S DGCONF=$P(DGX1,";"),DG=$$ISVALID^SISP6CL2(.DGCONF,.DGPOSS) I $G(DGCONF(DGCONF,"VEDIT")) S DIR(0)=DIR(0)_$P(DGX1,";",2)_":"_$P(DGX1,";",3)_";"
 ...
 . I $P(DGX1,";",2)=4 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG321,1)_$J("",6)_$E($$DAT^SISP6CL(DG321,4,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG321,5,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=5 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,1)_$J("",6)_$E($$DAT^SISP6CL(DG322,2,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,3,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=6 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,4)_$J("",6)_$E($$DAT^SISP6CL(DG322,5,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,6,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=7 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,7)_$J("",6)_$E($$DAT^SISP6CL(DG322,8,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,9,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=8 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,10)_$J("",6)_$E($$DAT^SISP6CL(DG322,11,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,12,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=9 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,16)_$J("",6)_$E($$DAT^SISP6CL(DG322,17,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,18,11)_$J("",12),1,12)
 . I $P(DGX1,";",2)=10 S DIR("A",DGCT)=DIR("A",DGCT)_$$YN^SISP6CL(DG322,19)_$J("",6)_$E($$DAT^SISP6CL(DG322,20,13)_$J("",12),1,12)_$E($$DAT^SISP6CL(DG322,21,11)_$J("",12),1,12)
 ;
 Q
 ;

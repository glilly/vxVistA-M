...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".32;16",DV="DX",DU="",DLB="NNTL-EOD",DIFLD=.3297
...
X1 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER2^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=3 D POS^DGINP
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".32;17",DV="DX",DU="",DLB="NNTL-RAD",DIFLD=.3298
...
X3 S DFN=DA K:X=$O(^DIC(23,"B","B.E.C.","")) X I $D(X) D SER2^DGLOCK S DGCOMBR=$G(Y) Q
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".32;14",DV="P25'X",DU="",DLB="NNTL-Char. Ser.",DIFLD=.3295
 S DE(DW)="C4^DVBHCE6"
 S DU="DIC(25,"
 G RE
...
X9 S DVBJC2=$S($D(^DPT(D0,.32)):$P(^(.32),U,3),1:"")
 Q
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW=".32;3",DV="*P21'X",DU="",DLB="PERIOD OF SERVICE",DIFLD=.323
 S DE(DW)="C10^DVBHCE6"
 S DU="DIC(21,"
 G RE

...
X10 S DVBSCR=1 D ^DVBHS5 S DVBJ2=1
 Q
11 S DW="TYPE;1",DV="P391'",DU="",DLB="TYPE",DIFLD=391
...
X12 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".3;1",DV="SX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C13^DVBHCE7"
 S DU="Y:YES;N:NO;"
 G RE
...
C16S S X="" G:DG(DQ)=X C16F1 K DB
 D ^DVBHCE12
C16F1 N X,X1,X2 S DIXR=840 D C16X1(U) K X2 M X2=X D C16X1("O") K X1 M X1=X
 D

...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".32;19",DV="SX",DU="",DLB="Service NTL Episode",DIFLD=.3285
 S DE(DW)="C1^DVBHCE3"
 S DU="Y:YES;N:NO;"
 G RE
...
X2 I $P(^DPT(D0,.32),U,19)'="Y" S Y="@31"
 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".32;11",DV="DX",DU="",DLB="NTL-EOD",DIFLD=.3292
...
X3 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER1^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=2 D POS^DGINP
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".32;12",DV="DX",DU="",DLB="NTL-RAD",DIFLD=.3293
...
X5 S DFN=DA K:X=$O(^DIC(23,"B","B.E.C.","")) X I $D(X) D SER1^DGLOCK S DGCOMBR=$G(Y) Q
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".32;9",DV="P25'X",DU="",DLB="NTL-Char. Ser.",DIFLD=.329
 S DE(DW)="C6^DVBHCE3"
 S DU="DIC(25,"
 G RE
...
X12 S DVBSCR=1 D ^DVBHS4 W !,?34,DVBON,"[NNTLAST]",DVBOFF X DVBLIT1
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".32;20",DV="SX",DU="",DLB="Service NNTL Episode",DIFLD=.32945
 S DE(DW)="C13^DVBHCE3"
 S DU="Y:YES;N:NO;"
 G RE

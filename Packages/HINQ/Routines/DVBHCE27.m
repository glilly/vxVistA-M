...
X13 I $P(DVBP(6),U,4)[" " S Y="@45",JP=JP+1
 Q
14 S DW=".321;1",DV="SX",DU="",DLB="VIETNAM SERVICE INDICATED?",DIFLD=.32101
 S DE(DW)="C14^DVBHCE27"
...
X25 S $P(^DPT(DFN,.3),U,2)="",$P(^DPT(DFN,.3),U,14)=""
 Q
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,DW=".3;2",DV="NJ3,0X",DU="",DLB="SERVICE CONNECTED PERCENTAGE",DIFLD=.302
 S DE(DW)="C26^DVBHCE27"
 S X=+$G(DVBDXPCT)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X26 S DFN=DA D EV^DGLOCK Q:'$D(X)  K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X),$D(^DPT(DA,.3)),$P(^(.3),U,1)'="Y" W !?4,*7,"Only applies to service-connected applicants." K X
 Q
 ;
27 D:$D(DG)>9 F^DIE17 G ^DVBHCE30

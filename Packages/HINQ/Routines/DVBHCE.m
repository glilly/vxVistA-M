DVBHCE ; GENERATED FROM 'DVBHINQ UPDATE' INPUT TEMPLATE(#960), FILE 2;03/19/13
 D DE G BEGIN
...
X18 S DVBJC=$S('$D(^DPT(DFN,.32)):"",1:$P(^(0),U,5))
 Q
19 S DW=".32;6",DV="DX",DU="",DLB="L-EOD",DIFLD=.326
...
X19 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SV^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=1 D POS^DGINP
 Q
 ;
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".32;7",DV="DX",DU="",DLB="L-RAD",DIFLD=.327
...
X22 I X'=DVBJC S DVBJ2=1
 Q
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW=".32;4",DV="P25'X",DU="",DLB="L-Char. Ser.",DIFLD=.324
 S DE(DW)="C23^DVBHCE"
 S DU="DIC(25,"
 G RE

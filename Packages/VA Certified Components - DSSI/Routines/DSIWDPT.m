DSIWDPT ;DSS/SGM/LM - DESKTOP PATIENT LOOKUP
 ;;1.0;;;;
 ;
 ...
 ;
LOOKUP(DSIWDPT,DSIWA) ; RPC: DSIW PATIENT LOOKUP
 ; DSIWA(n) - req where DSIWA(n) = code ^ value for n=1,2,3,...
 ; 
 ...
 ;
 ;RETURN DESCRIPTION
 ; DSIWDPT(n) for n=1,2,3,4,...
 ; If no matches found, then DSIWDPT(1) = -1^message
 ; Otherwise, DSIWDPT(n) = p1^p2^p3^p4^p5^p6^... where
 ...
 ;         mimics the behavior of the DSICFM and DSICDDR calls)
 ;
 ...
 ;
 ;
 D LOOKUP^DSIWDPT1(.DSIWDPT,.DSIWA)
 Q

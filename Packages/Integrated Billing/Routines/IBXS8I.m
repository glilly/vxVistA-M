IBXS8I ; GENERATED FROM 'IB SCREEN82' INPUT TEMPLATE(#577), FILE 399;03/19/13
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("U2")) S %Z=^("U2") S %=$P(%Z,U,1) S:%]"" DE(17)=% S %=$P(%Z,U,8) S:%]"" DE(12)=% S %=$P(%Z,U,9) S:%]"" DE(14)=% S %=$P(%Z,U,10) S:%]"" DE(29)=%
...
C29F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=843 S DIEZRXR(399,DIXR)=""
 Q
X29 S DIC("S")="I $P(^(0),U,2)=1,$P(^(0),U)'["",""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
31 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=31 S I(0,0)=D0 S Y(1)=$S($D(^DGCR(399,D0,"U2")):^("U2"),1:"") S X=$P(Y(1),U,10),X=X S D(0)=+X S X=$S(D(0)>0:D(0),1:"")
 S DGO="^IBXS8I4",DC="^355.93^IBA(355.93," G DIEZ^DIE0
R31 D DE G A
 ;

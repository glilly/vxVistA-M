...
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;7",DV="NJ3,0XOa",DU="",DLB="COUNTY",DIFLD=.117
 S DQ(2,2)="S Y(0)=Y Q:Y']""""  S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),""^"",5),1:"""") Q:'Z0  S Y=$P($S($D(^DIC(5,Z0,1,Y,0)):^(0),1:""""),""^"",3) S:$T(COUNTY^VFDCDPT)'="""" Y=$$COUNTY^VFDCDPT(Z0,Y(0))"
 S DE(DW)="C2^IBXSC114"
 G RE
...
X3 K:$L(X)>20!($L(X)<4) X
 ...
 ;
4 S DQ=5 ;@155
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".121;9",DV="SX",DU="",DLB="TEMPORARY ADDRESS ACTIVE?",DIFLD=.12105

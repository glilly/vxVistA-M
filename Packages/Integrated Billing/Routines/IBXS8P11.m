...
DE S DIE="^DGCR(399,D0,""PRV"",",DIC=DIE,DP=399.0222,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"PRV",DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(6)=% S %=$P(%Z,U,7) S:%]"" DE(10)=% S %=$P(%Z,U,12) S:%]"" DE(1)=% S %=$P(%Z,U,13) S:%]"" DE(5)=% S %=$P(%Z,U,14) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXS8P11",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;12",DV="*P355.97'",DU="",DLB="PRIM INS PROVIDER ID TYPE",DIFLD=.12
 S DE(DW)="C1^IBXS8P11"
 S DU="IBE(355.97,"
 S X=DIPA("PRIDT")
 S Y=X
...
C1F1 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;5",DV="FX",DU="",DLB="PRIMARY INS CO ID NUMBER",DIFLD=.05
 S DE(DW)="C2^IBXS8P11"
 S X=DIPA("PRID")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X3 S Y="@8382"
 Q
4 S DQ=5 ;@8372
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;13",DV="*P355.97'",DU="",DLB="SEC INS PROVIDER ID TYPE",DIFLD=.13
 S DE(DW)="C5^IBXS8P11"
 S DU="IBE(355.97,"
 S X=DIPA("PRIDT")
 S Y=X
...
C5F1 Q
X5 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;6",DV="FX",DU="",DLB="SECONDARY INS CO ID NUMBER",DIFLD=.06
 S DE(DW)="C6^IBXS8P11"
 S X=DIPA("PRID")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X7 S Y="@8383"
 Q
8 S DQ=9 ;@8373
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;14",DV="*P355.97'",DU="",DLB="TERT INS PROVIDER ID TYPE",DIFLD=.14
 S DE(DW)="C9^IBXS8P11"
 S DU="IBE(355.97,"
 S X=DIPA("PRIDT")
 S Y=X
...
C9F1 Q
X9 Q
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW="0;7",DV="FX",DU="",DLB="TERTIARY INS CO ID NUMBER",DIFLD=.07
 S DE(DW)="C10^IBXS8P11"
 S X=DIPA("PRID")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z

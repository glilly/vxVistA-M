...
X11 K:$L(X)>30!($L(X)<1) X I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 ...
 Q
 ;
12 S DW="0;10",DV="F",DU="",DLB="STREET ADDRESS LINE 2",DIFLD=.1
 G RE
X12 K:$L(X)>30!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
...
X26 S:$D(^IBA(355.93,DIPA("NVA_PRV"),"TAXONOMY"))>0 Y="@837"
 Q
27 S D=0 K DE(1) ;42
 S DIFLD=42,DGO="^IBXS8P13",DC="3^355.9342IP^TAXONOMY^",DV="355.9342M*P8932.1'",DW="0;1",DOW="TAXONOMY CODE",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="USC(8932.1,"

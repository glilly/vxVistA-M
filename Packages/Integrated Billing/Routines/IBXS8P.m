IBXS8P ; GENERATED FROM 'IB SCREEN8H' INPUT TEMPLATE(#515), FILE 399;03/19/13
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("U2")) S %Z=^("U2") S %=$P(%Z,U,1) S:%]"" DE(8)=% S %=$P(%Z,U,8) S:%]"" DE(18)=% S %=$P(%Z,U,9) S:%]"" DE(20)=% S %=$P(%Z,U,10) S:%]"" DE(27)=%
...
C27F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X27 S DIC("S")="I $P(^(0),U,2)=1,$P(^(0),U)'["",""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;

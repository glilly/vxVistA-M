XINDX6 ;ISC/REL,GRK - GET SET OF ROUTINES TO INDEX ;07/22/08  13:54
 ;;7.3;TOOLKIT;**20,27,66,110**;Apr 25, 1995;Build 86
...
R1 R !,"ROUTINE NAME: ",ROU:$S($G(DTIME):DTIME,1:360) Q:ROU=""
 ...
 G R1
 ;
VFD(RL) ;Quick Report, Just errors on some routines
 ; clone QUICK^XINDEX() module, preanswer as many questions as possible
 ; RL - opt - list of routine separated by a comma
 ;            I $D(RL) do not ask for routines or Builds or packages 
 N %A2,%I,%IN2,%IN3,%N,%QMK,%YN,AC,ANS,C8,CM,CX,DEF,DDOT,DIF,E,EC,ER
 N INDHDR,INDXDT,INP,LI,LL,LN,LV,N,NOA,OP,PG,QUES,RN,T,XCNP,XX1,XX2,Z,Z1
 N I,X,DA,INDDA,IOP,NRO,ZTDTH,ZTIO,ZTSK
 K ^UTILITY($J)
 S:'$D(DTIME)#2 DTIME=360
 D HOME^%ZIS,HDR^XINDX7
 I $D(RL) F %I=1:1 S Z=$P(RL,",",%I) Q:Z=""  S ^UTILITY($J,Z)=""
 D PARAM,ASKRTN:'$D(RL)
 I $O(^UTILITY($J,"@"))="" W !,"No Routines to process.",! D ^%ZISC Q
 S INP(1)=1,INP(6)=1 ; more than error, more than warnings
 I '$D(RL),$D(^DIC(9.4))!$D(^DIC(9.6)) D ^XINDX10 G END:$D(DUOUT) S INDDA=DA
 S NRO=0,X=0 F I=0:0 S X=$O(^UTILITY($J,X)) Q:X=""  S NRO=NRO+1
 G END:(NRO'>0)&(INDDA'>0)
 S IOP=";P-OTHER;80;99999",%ZIS="" D ^%ZIS
 G ALIVE^XINDEX

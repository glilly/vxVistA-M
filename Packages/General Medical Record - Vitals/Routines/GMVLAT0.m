...
EN1(DFN) ; Entry to extract the latest vitals/measurements for a patient
 ...
 S GJ=0,GBLANK=""
 ;DSS/LM - Modify loop to include vxVistA additional vital types - 2/16/2010 Add LEN
 ;F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN" I $D(^GMRD(120.51,"C",X))  S GMR(X)=$O(^GMRD(120.51,"C",X,"")),Y=$P($G(^GMRD(120.51,GMR(X),0)),"^") Q:Y=""
 F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN","LMP","EDD","LEN" I $D(^GMRD(120.51,"C",X))  S GMR(X)=$O(^GMRD(120.51,"C",X,"")),Y=$P($G(^GMRD(120.51,GMR(X),0)),"^") Q:Y=""
 ...
 F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN","LMP","EDD","LEN" I $D(GMRDATA(X)) S GMRVDT="",(GMRVDT(1),GMVD)=0 F  S GMVD=$O(GMRDATA(X,GMVD)) Q:GMVD'>0  D WRTDT S GMVD(1)=0 F  S GMVD(1)=$O(GMRDATA(X,GMVD,GMVD(1))) Q:GMVD(1)'>0  D
 ...
 ...Q
 ..Q
 .;DSS/LM - Return LMP and EDD (formatted)
 .I X="LMP"!(X="EDD") S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_$$FMTE^XLFDT(GMRDATA(X,GMVD,GMVD(1)),"5D")
 .; 2/16/2010 - Add LEN
 .I X="LEN" S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRDATA(X,GMVD,GMVD(1))

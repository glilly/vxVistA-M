...
EN1 ;CALLED FROM GMVGETD ENTRY POINT GETVM 
 K ^TMP($J,"GMRV")
 ;DSS/LM - Begin modification - Accommodate LMP and EDD - 2/16/2010 Add LEN
 ;F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN"  S GMRVITY=$O(^GMRD(120.51,"C",GMRVTY,0)) I GMRVITY>0 D SETAR
 F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN","LMP","EDD","LEN"  S GMRVITY=$O(^GMRD(120.51,"C",GMRVTY,0)) I GMRVITY>0 D SETAR
 ;DSS/LM - End modificaton
...
PRT ;SAVE V/M BY DATE/TIME
 ...
 F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN","LMP","EDD","LEN" I $D(^TMP($J,"GMRV",GMRDATE,GMRVTY)) F GMRVDA=0:0 S GMRVDA=$O(^TMP($J,"GMRV",GMRDATE,GMRVTY,GMRVDA)) Q:GMRVDA'>0  D SETLN^GMVGETD2
 ;DSS/LM - End modificaton
 Q

PSOLBLN ;BIR/RTR-NEW PRINTS LABEL ;11/18/92
 ;;7.0;OUTPATIENT PHARMACY;**16,36,71,107,110,117,135,233**;DEC 1997;Build 153
...
L1 ;
 ;DSS/SGM - BEGIN MODS to remove references to VAMC
 ; original code is now follows the 'E  ' statement
 I $G(VFDPSOLB) D VAMC^VFDPSOLB("LBLN")
 E  W ?3,"VAMC ",$P(PS,"^",7),", ",STATE,"  ",$G(PSOHZIP),?54,"VAMC ",$P(PS,"^",7),", ",STATE,"  ",$G(PSOHZIP),?102 W $S($D(REPRINT)&($G(PSOBLALL)):"(GROUP REPRINT)",$D(REPRINT):"(REPRINT)",1:"") W:$G(RXP) "(PARTIAL)"
 ;DSS/SGM - END MODS
...
L11 W !?54,"Last fill prior to ",$G(EXPDT),?102,"Mfg "_$G(MFG)_" Lot# "_$G(LOT)
 ;DSS/SGM - BEGIN MODS - don't display copay message, move orig to L12+1
L12 I $G(VFDPSOLB) D COPAY^VFDPSOLB("LBLN")
 E  W !,$P(PS,"^",2),?54,$S($L($G(COPAYVAR)):$G(COPAYVAR)_"     ",1:""),"Days Supply: ",$G(DAYS),?102,"Tech__________RPh_________",!,$P(PS,"^",7),", ",STATE,"  ",$G(PSOHZIP)
 ;DSS/SGM - END MODS
 ...
 ;DSS/SGM - BEGIN MODS - remove patient status, orig is ELSE argument
 I $G(VFDPSOLB) D PSTAT^VFDPSOLB("LBLN")
 E  W !,$S($D(PSMP(1)):PSMP(1),1:$G(VAPA(1))),?54,"[ ] Permanent",?102,"Pat. Stat ",PATST," Clinic: ",PSCLN
 ;DSS/SGM - END MODS

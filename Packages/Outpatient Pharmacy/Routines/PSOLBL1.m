PSOLBL1 ;BHAM ISC/SAB - PRINTS LABEL ;1/20/93 14:25
 ;;7.0;OUTPATIENT PHARMACY;**107,110,225,326**;DEC 1997;Build 7
START S COPIES=COPIES-1
 ;DSS/SGM - BEGIN MODS - Conditionally remove VA (119), orig code argument of ELSE
 I $G(VFDPSOLB) D SVC^VFDPSOLB("LBL1") I 1
 E  W $C(13) S $X=0 W "VA (119)",?10,$E(DT,4,5),"/",$E(DT,6,7),"/",$E(DT,2,3) W:('SIDE)&(PRTFL) ?40,"PLEASE REFER ONLY TO '",$S(REF:"1. REFILL REQUEST",1:"2. RENEWAL ORDER"),"'" W:+$G(RXP) ?100,"(PARTIAL)" W:$D(REPRINT) ?110,"(REPRINT)"
 ;DSS/SGM - END MODS
 ...
 ;DSS/SGM - BEGIN MODS - Conditionally remove reference to VA Medical Center in next
 ;W !,?8,"VA Medical Center" I 'SIDE W ?38,INT(1)
 W !?8 W:'$G(VFDPSOLB) "VA Medical Center" I 'SIDE W ?38,INT(1)
 ;DSS/SGM - END MODS

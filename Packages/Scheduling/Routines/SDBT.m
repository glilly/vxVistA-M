SDBT ; GENERATED FROM 'SDB' INPUT TEMPLATE(#485), FILE 44;05/05/06
 D DE G BEGIN
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^SC(DA,""))=""
 ...
 I  S %=$P(%Z,U,21) S:%]"" DE(7)=% S %=$P(%Z,U,25) S:%]"" DE(14)=%
 I $D(^(99)) S %Z=^(99) S %=$P(%Z,U,1) S:%]"" DE(15)=%
 I $D(^("AT")) S %Z=^("AT") S %=$P(%Z,U,1) S:%]"" DE(13)=%
 I $D(^("LTR")) S %Z=^("LTR") S %=$P(%Z,U,1) S:%]"" DE(18)=% S %=$P(%Z,U,2) S:%]"" DE(19)=% S %=$P(%Z,U,3) S:%]"" DE(20)=%
...
4 S DW="0;8",DV="RS",DU="",DLB="SERVICE",DIFLD=9
 ...
 G RE
X4 Q
5 S DW="0;17",DV="RS",DU="",DLB="NON-COUNT CLINIC? (Y OR N)",DIFLD=2502
 S DE(DW)="C5^SDBT"
 S DU="Y:YES;N:NO;"
 G RE

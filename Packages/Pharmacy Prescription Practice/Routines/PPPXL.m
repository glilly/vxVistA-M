PPPXL ; GENERATED FROM 'PPP LOGP' PRINT TEMPLATE (#508) ; 07/22/96 ; (FILE 1020.4, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(508,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 ...
 S X=$G(^PPP(1020.4,D0,0)) D N:$X>40 Q:'DN  W ?40 S Y=$P(X,U,4) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,35)

ORWPCE1 ;SLC/KCM - PCE Calls from CPRS GUI; 10/26/04 ;11/20/09  09:25
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**10,85,116,132,148,187,190,215,243,280**;Dec 17, 1997;Build 7
 ;
...
DQSAVE ; Background Call to DATA2PCE
 ...
 . ;DSS/SMP - BEGIN MODS - Incorporate vxVistA tabs from CPRS/PCE
 . I $G(^%ZOSF("ZVX"))["VX",$E(TYP,1,6)="VFDSNO" D  ;SnoMed codes.
 . . Q:'$L(CODE)
 . . S VFDSNO=VFDSNO+1,ROOT="ORPXAPI(""VFDSNO"","_VFDSNO_")"
 . . S @ROOT@("VFDSNO CODE")=CODE
 . . S:$L(NARR) @ROOT@("NARRATIVE")=NARR
 . . S:$L(CAT) @ROOT@("CATEGORY")=CAT
 . . S:$L($P(X,U,5)) @ROOT@("USAGE")=$P(X,U,5)
 . . S:$P(X,U,6)'>0 $P(X,U,6)=$G(ORPXAPI("PROVIDER",1,"NAME"))
 . . S:$P(X,U,6)>0 @ROOT@("ENC PROVIDER")=$P(X,U,6)
 . . S:$L($P(X,U,10))>0 COMMENT($P(X,U,10))=$E(TYP,1,6)_"^"_VFDSNO
 . . I $E(TYP,7)="-" S @ROOT@("DELETE")=1
 . ;DSS/SMP - END MODS
 ;Store the comments
 S COM=""
...
DATA2PCE ;
 ...
 ;DSS/SMP - BEGIN MODS
 I $D(VFD)>1,OK>0,ORAVST>0 S VFD(1+$O(VFD(""),-1))="VISIT^"_ORAVST D EN^VFDCPX(,.VFD)
 ;DSS/SMP - END MODS
 S ZTSTAT=0  ; clear sync flag
 Q
 ;

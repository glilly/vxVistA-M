...
C1X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C1F2 S DIXR=418 D C1X2(U) K X2 M X2=X D C1X2("O") K X1 M X1=X
 K X M X=X2 D
 . N DIEXARR M DIEXARR=X S DIEZCOND=1
 . S X=$$GET^XPAR("SYS","VFD DPT ONE RACE ONLY",1,"Q")
 . S DIEZCOND=$G(X) K X M X=DIEXARR Q:'DIEZCOND
 . N VFDA M VFDA=DA N D0,D1,DA,DIC,DIE,DR,DINUM,VFDER,VFD,VFDI,VFDIEN S VFDI=0 F  S VFDI=$O(^DPT(VFDA(1),.02,VFDI)) Q:'VFDI  I VFDI'=VFDA S VFDIEN=VFDI_","_VFDA(1)_"," S VFD(2.02,VFDIEN,.01)="@" D FILE^DIE(,"VFD","VFDER") K VFD,VFDER
 G C1F3
C1X2(DION) K X
 S X(1)=$G(@DIEZTMP@("V",2.02,DIIENS,.01,DION),$P($G(^DPT(DA(1),.02,DA,0)),U,1))
 S X=$G(X(1))

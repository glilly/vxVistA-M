DGPMX3 ; GENERATED FROM 'DGPM DISCHARGE' INPUT TEMPLATE(#448), FILE 405;03/19/13
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(12)=%
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,2) S:%]"" DE(15)=% S %=$P(%Z,U,3) S:%]"" DE(17)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X7 S:$S(X=27:0,X=32:0,1:1) Y="@99"
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 S I(0,0)=D0 S Y(1)=$S($D(^DGPM(D0,0)):^(0),1:"") S X=$P(Y(1),U,3),X=X S D(0)=+X S X=$S(D(0)>0:D(0),1:"")
 S DGO="^DGPMX31",DC="^2^DPT(" G DIEZ^DIE0
R8 D DE G A
 ;
9 S DQ=10 ;@99
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 S ^DISV(DUZ,"DGPM3")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 I $P(^DGPM(DA,0),"^",18)'=10 S Y="@31"
 Q
...
X14 S:'DGODS Y="@31"
 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="ODS;2",DV="S",DU="",DLB="NON-VA FACILITY?",DIFLD=11500.02
 S DE(DW)="C15^DGPMX3"
 S DU="1:YES;0:NO;"
 G RE
...
X16 S Y=$S('DGODSPT:11500.05,'X:"@31",1:11500.03)
 Q
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW="ODS;3",DV="S",DU="",DLB="AT VA EXPENSE?",DIFLD=11500.03
 S DU="1:YES;0:NO;"
 G RE
X17 Q

DGPMX1 ; GENERATED FROM 'DGPM ADMIT' INPUT TEMPLATE(#446), FILE 405;06/26/13
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(10)=% S %=$P(%Z,U,11) S:%]"" DE(7)=% S %=$P(%Z,U,12) S:%]"" DE(4)=%

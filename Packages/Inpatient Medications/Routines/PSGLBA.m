PSGLBA ;BIR/CML3-LABEL ALIGNMENT ; 10 Nov 2009  3:42 PM
 ;;5.0; INPATIENT MEDICATIONS ;;16 DEC 97;Build 153
 ;
ENLP ; please do NOT place an IF statement in this paragraph
 ;DSS/SGM - BEGIN MODS - zebra label printing
 N VFDZ D  Q:$G(VFDZ)
 .U IO I $L($T(EN^VFDPSPRT)),$$EN^VFDPSPRT("TEST","PSGLOI") S VFDZ=1
 .Q
 ;DSS/SGM - END MODS
...
L2 W !!," XX/XX | XX/XX | XX/XX/XX  XX:XX  (PXXXX)  | A T",?52,"PATIENT NAME",?87,"ROOM-BED"
 ...
 W !," WS HSM NF",?25,"RPH:_____ RN:_____",?43,"| N S",?52,"WARD GROUP",?91,"WARD",!!
 Q
 ;
...
ALDONE ;
 ...
 ;
DEV ;
 ;DSS/JG - BEGIN MODS - MAR ALIGNMENT LABEL PRINT FOR ZEBRA
 ;I '$D(IO("Q")) F  D ENLP,OK Q:Y
 I '$D(IO("Q")) N IOP,VFDIOP S VFDIOP=ION F  D ENLP,^%ZISC,OK Q:Y  S IOP=VFDIOP D ^%ZIS
 ; DSS/JG - END MODS
...
OK ;
 ...
 S Y=%<2 Q:Y  W !!,"Re-align the labels, then press the RETURN key for more test labels." R X:DTIME W:'$T $C(7) S:X="^"!'$T Y=1 Q
 ;
H2 ;
